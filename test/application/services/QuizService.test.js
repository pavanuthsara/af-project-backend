...
describe('QuizService', () => {
  ...
  describe('User Methods', () => {
    ...
    describe('submitQuiz', () => {
      ...
      test('should replace static explanations with AI-generated ones', async () => {
        const submittedAnswers = [
          { questionId: VALID_QUESTION_ID_1, selectedOption: 'A' },
          { questionId: VALID_QUESTION_ID_2, selectedOption: 'C' }
        ];
        ...
        const mockQuestions = [
          { _id: VALID_QUESTION_ID_1, correctAnswer: 'A', explanation: 'Static 1', questionText: 'Q1?', options: ['A', 'B'] },
          { _id: VALID_QUESTION_ID_2, correctAnswer: 'B', explanation: 'Static 2', questionText: 'Q2?', options: ['B', 'C'] }
        ];

        const mockAttempt = { save: jest.fn().mockResolvedValue(true) };

        Quiz.findById = jest.fn().mockResolvedValue(mockQuiz);
        const mockQuery = { select: jest.fn().mockResolvedValue(mockQuestions) };
        Question.find = jest.fn().mockReturnValue(mockQuery);
        User.findById = jest.fn().mockResolvedValue(null);
        QuizAttempt.mockImplementation(() => mockAttempt);

        mockExplanationService.generateExplanations = jest.fn().mockResolvedValue([
          'AI explanation for Q2'
        ]);

        const result = await quizService.submitQuiz(VALID_USER_ID, VALID_QUIZ_ID, submittedAnswers);

        expect(mockExplanationService.generateExplanations).toHaveBeenCalledTimes(1);
        expect(result.wrongAnswerExplanations).toHaveLength(1);
        expect(result.wrongAnswerExplanations[0].explanation).toBe('AI explanation for Q2');
      });

      test('should keep static explanations when AI generation fails', async () => {
        const submittedAnswers = [
          { questionId: VALID_QUESTION_ID_1, selectedOption: 'B' }
        ];
        ...
        const mockQuestions = [
          { _id: VALID_QUESTION_ID_1, correctAnswer: 'A', explanation: 'Static fallback', questionText: 'Q1?', options: ['A', 'B'] }
        ];
        ...
        mockExplanationService.generateExplanations = jest.fn().mockRejectedValue(
          new Error('Groq API down')
        );

        const result = await quizService.submitQuiz(VALID_USER_ID, VALID_QUIZ_ID, submittedAnswers);

        // Should gracefully fall back to static explanation
        expect(result.wrongAnswerExplanations).toHaveLength(1);
        expect(result.wrongAnswerExplanations[0].explanation).toBe('Static fallback');
      });

      test('should work without ExplanationService (backward compatible)', async () => {
        const serviceWithout = new QuizService(); // no explanationService
        ...
        const mockQuestions = [
          { _id: VALID_QUESTION_ID_1, correctAnswer: 'A', explanation: 'Static only', questionText: 'Q1?', options: ['A', 'B'] }
        ];
        ...
        const result = await serviceWithout.submitQuiz(VALID_USER_ID, VALID_QUIZ_ID, submittedAnswers);

        expect(result.wrongAnswerExplanations[0].explanation).toBe('Static only');
      });
    });
